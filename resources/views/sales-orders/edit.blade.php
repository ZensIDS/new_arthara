...
<div
    x-data="soEditForm(
        ...
        {{ Illuminate\Support\Js::from(old('other_costs', $salesOrder->otherCosts->map(fn ($cost) => [
...
>
    <div class="mb-6">
        ...
        {{--  <p class="text-sm text-ink/50 mt-1">
            Mengubah item akan mengembalikan alokasi stok FIFO lama, lalu mengalokasikan ulang
            sesuai perubahan. Transaksi ini tetap bisa diedit meskipun sudah ada pembayaran —
            hanya diblokir kalau barangnya sudah pernah diretur customer.
        </p>
    ...
    <form method="POST" action="{{ route('sales-orders.update', $salesOrder) }}" @submit="onSubmit">
        ...
        <div class="rounded-2xl border border-ink/10 bg-white shadow-card overflow-hidden mb-6">
            <div class="px-6 py-4 border-b border-ink/10 flex items-center justify-between">
                <div>
                    ...
                    <p class="text-xs text-ink/50 mt-0.5">Opsional — packing, ongkir, dll. Otomatis tercatat sebagai Pengeluaran, tidak menambah Total Transaksi di atas.</p>
                </div>
        ...
        <div
            x-data="{ packingCost: {{ (int) old('estimated_packing_cost', $salesOrder->estimated_packing_cost) }} || '' }"
            class="rounded-2xl border border-amber-200 bg-amber-50/40 shadow-card p-6 mb-6"
        >
            <div class="flex items-center gap-2 mb-1">
                ...
                <h3 class="font-display font-semibold">Estimasi Biaya Packing</h3>
            </div>
            <p class="text-xs text-ink/50 mb-4">Opsional — hanya untuk gambaran untung-rugi di halaman detail & daftar SO, tidak mempengaruhi total transaksi maupun catatan keuangan lain.</p>
            <div class="max-w-xs">
                <div class="relative">
                    <span class="absolute left-3.5 top-1/2 -translate-y-1/2 text-sm text-ink/40">Rp</span>
                    <input type="text" inputmode="numeric"
                           :value="formatRupiah(packingCost)"
                           @input="packingCost = parseRupiah($event.target.value); $event.target.value = formatRupiah(packingCost)"
                           class="w-full rounded-xl border border-ink/12 pl-9 pr-3.5 py-2.5 text-sm tnum bg-white focus:outline-none focus:border-amber-500 focus:ring-4 focus:ring-amber-500/15 transition-shadow">
...
@push('scripts')
<script>
    function soEditForm(customers, products, sources, initialItems, expenseCategories, initialOtherCosts) {
        return {
            customers,
            products,
            sources,
            expenseCategories,
            sourceId: {{ (int) old('source_id', $salesOrder->source_id) }},
            ...
            // dipilih berulang di baris lain, jadi tidak perlu guard anti-duplikat.
